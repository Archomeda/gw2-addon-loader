    ...
    namespace addons {
        ...
        bool ProxyAddon::Initialize() {
            ...
            if (h == NULL) {
                ...
                GetLog()->error("Could not initialize proxy addon {0}: Library handle is empty", this->GetFileName());
                return false;
            }
            ...
            if (this->ProxyInitialize == nullptr) {
                this->ChangeState(AddonState::ErroredState);
                GetLog()->error("Could not initialize proxy addon {0}: Addon doesn't contain a valid ProxyInitialize export", this->GetFileName());
                return false;
            }
            if (this->ProxyRelease == nullptr) {
                this->ChangeState(AddonState::ErroredState);
                GetLog()->error("Could not initialize proxy addon {0}: Addon doesn't contain a valid ProxyRelease export", this->GetFileName());
                return false;
            }
