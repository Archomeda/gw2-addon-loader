            ...
            ImGui::BeginGroup();
            {
                ...
                ImGui::BeginChild("##Addon", ImVec2(0, -ImGui::GetFrameHeightWithSpacing()));
                {
                    ...
                    if (addon->IsForced()) {
                        ImGui::Spacing();
                        ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(255, 204, 0, 255));
                        ImGui::TextWrapped("This is a supportive add-on for the Add-on Loader and cannot be disabled.");
                        ImGui::PopStyleColor();
                    }
...
Add-ons have to be compatible before they can be used with the Add-on Loader. While incompatible add-ons will still show up in the list of add-ons, you can't activate them. If you want to use an add-on that's not compatible, ask its developer to update it.
